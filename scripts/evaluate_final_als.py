"""Refit ALS through validation and evaluate once on the test split."""
...
ARTIFACT_DIR = Path("data/artifacts") / DATASET / "final" / "als"
RESULTS_PATH = Path("results") / f"{DATASET}_als_test.json"
K = 10


def main() -> None:
    """Fit on train plus validation and evaluate untouched test events."""
    ...
    print(
        f"Training ALS on {len(fit_events):,} interactions "
        f"through validation..."
    )

    training_start = perf_counter()
    model = ALSRecommender.fit(fit_events, config=config)
    ...
    users_per_second = (
        len(warm_test_users) / recommendation_seconds
    ...
    results = {
        ...
        "warm_evaluated_users": len(warm_test_users),
    ...
    print("Model:         final implicit ALS")
